...
public class MemberDAO {
	// singleton
	private static MemberDAO dao = new MemberDAO(); //static타입이라는 것을 기억해.
	...
	public ArrayList<MemberDTO> selectMemberAll() throws SQLException {
		ArrayList<MemberDTO> mem = new ArrayList<MemberDTO>(); //new 해줄 때는 무조건 앞에꺼랑 똑같이 () 해줘야됨
		...
		while (rs.next()) {
			mem.add(new MemberDTO(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
					rs.getString(6), rs.getString(7))); //arrayList니까 add로 넣어줘야함
		}
		return mem;
	...
	public int insertMember(MemberDTO dto) throws SQLException {
		...
		String sql = "insert into Members(userId,userPwd,userName,phoneNo,address,email) values(?,?,?,?,?,?)";
		PreparedStatement pstmt = getConnection().prepareStatement(sql); // db에 insert
	...
	public int updateMember(MemberDTO dto) throws SQLException {
		String sql = "update Member2 set userId=? where userId=?";
		PreparedStatement pstmt = getConnection().prepareStatement(sql);
		pstmt.setString(1, dto.getUserId());
